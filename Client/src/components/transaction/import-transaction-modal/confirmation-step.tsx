...
import { MAX_IMPORT_LIMIT, _PaymentMethodsEnum, _TransactionType, BulkTransactionType } from "@/@types/transaction/transactionTypes";
import { useProgressLoader } from "@/hook/use-progress-loader";
import { useBulkImportTransactionMutation } from "@/api/transaction/transactionApi";
...
const transactionSchema = z.object({
   ...
   paymentMethod: z
      .union([
         ...
         z.enum(
            [
               _PaymentMethodsEnum.CARD,
               _PaymentMethodsEnum.BANK_TRANSFER,
               _PaymentMethodsEnum.MOBILE_PAYMENT,
               _PaymentMethodsEnum.AUTO_DEBIT,
               _PaymentMethodsEnum.CASH,
               _PaymentMethodsEnum.OTHER,
            ],
            {
               errorMap: (issue) => ({
                  message:
                     issue.code === "invalid_enum_value"
                        ? `Payment method must be one of: ${Object.values(_PaymentMethodsEnum).join(", ")}`
                     ...
                     .map((e) => {
                        ...
                        if (e.path[0] === "paymentMethod")
                           return (
                              ...
                              Object.values(_PaymentMethodsEnum).join(", ")
                           );
