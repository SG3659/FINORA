...
import { DateRangeEnum, type DateRangePreset } from "../enums/date-range.js";
import TransactionModel, { TransactionTypeEnum } from "../model/transaction.model.js";
import { getDateRange } from "../utils/date.js";
import mongoose from "mongoose";
import { differenceInDays, subDays, subYears } from "date-fns";
import { calaulatePercentageChange } from "../utils/helper.js";
import { convertToRupee } from "../utils/format-currency.js";
export const summaryAnalyticsService = async (userId: string, customFrom?: Date, customTo?: Date, dateRangePreset?: DateRangePreset) => {
...
export const chartAnalyticsService = async (userId: string, customFrom?: Date, customTo?: Date, dateRangePreset?: DateRangePreset) => {
   const range = getDateRange(dateRangePreset, customFrom, customTo);
   ...
   const transaformedData = (resultData?.chartData || []).map((item: any) => ({
      date: item.date,
      income: convertToRupee(item.income),
      expenses: convertToRupee(item.expenses),
   }));
   return {
...
export const expensePieChartBreakdownService = async (userId: string, customFrom?: Date, customTo?: Date, dateRangePreset?: DateRangePreset) => {
   const range = getDateRange(dateRangePreset, customFrom, customTo);
   ...
   const transformedData = {
      totalSpent: convertToRupee(data.totalSpent),
      breakdown: data.breakdown.map((item: any) => ({
         ...item,
         value: convertToRupee(item.value),
      })),
   };
